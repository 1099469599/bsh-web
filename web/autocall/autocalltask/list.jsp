...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#autoCallTaskDlg").dialog({  //当任务表单弹窗关闭时执行的动作
				onClose:function() {
					currTaskId = null;
		...
		function showExtraTabs(flag) {   //是否显示额外Tab,主要是导入号码和号码列表
	    	if(flag==1) {    //flag=1 时，显示
	    		...
	    		if(currTaskType=="3") {   //催缴类型
	    			...
	    			$("#selectAutoNumberDiv").css("display","");
	    		}
		...
		function rowformatter(value,data,index) {
			return "<a href='#' onclick='javascript:autoCallTaskEdit(\"" + data.TASK_ID + "\",\"" + data.TASK_NAME + "\",\"" + data.CALLERID + "\",\"" + data.PLAN_START_TIME + "\",\"" + data.PLAN_END_TIME + "\",\"" + data.SCHEDULE_ID + "\",\"" + data.SCHEDULE_NAME + "\",\"" + data.TASK_TYPE + "\",\"" + data.COMMON_VOICE_ID + "\",\"" + data.COMMON_VOICE_DESC + "\",\"" + data.QUESTIONNAIRE_ID + "\",\"" + data.QUESTIONNAIRE_DESC + "\",\"" + data.REMINDER_TYPE + "\",\"" + data.START_VOICE_ID + "\",\"" + data.START_VOICE_DESC + "\",\"" + data.END_VOICE_ID + "\",\"" + data.END_VOICE_DESC + "\",\"" + data.BLACKLIST_ID + "\",\"" + data.BLACKLIST_NAME + "\",\"" + data.RETRY_TIMES + "\",\"" + data.RETRY_INTERVAL + "\",\"" + data.PRIORITY + "\")'><img src='themes/icons/pencil.png' border='0'>编辑</a>" + 
