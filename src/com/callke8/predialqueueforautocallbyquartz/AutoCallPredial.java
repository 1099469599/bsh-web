...
public class AutoCallPredial {
	...
	public void exec() throws SchedulerException {
		...
		//测试线程 test
		Scheduler schedulerForAutoFlowTest = QuartzUtils.createScheduler("AutoFlowTTSJobTest" + System.currentTimeMillis(),1);
		JobDetail jobDetailTest = QuartzUtils.createJobDetail(AutoTestJob.class);
		schedulerForAutoFlowTest.scheduleJob(jobDetailTest, QuartzUtils.createSimpleTrigger(new Date((System.currentTimeMillis() + 1000)), -1, 1000));   //执行一次
		schedulerForAutoFlowTest.start();
